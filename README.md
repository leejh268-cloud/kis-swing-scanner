...
스윙에서 성과가 좋은 경향이 있습니다. 최근 20거래일 수익률을 같은 기간 지수
(코스피/코스닥) 수익률과 비교해서, 지수를 못 이기는 종목은 신호에서 제외합니다.

끄고 싶으면 `scanner/rs_filter.py`의 `RS_MARGIN` 값을 아주 낮게(예: -100) 설정하면
사실상 모든 종목이 통과합니다. 더 엄격하게 하고 싶으면 `RS_MARGIN`을 양수로
올리세요 (예: `RS_MARGIN = 3.0` → 지수보다 최소 3%p 이상 더 강해야 통과).
...
- **상대강도**: 최근 20거래일 수익률이 같은 시장 지수보다 높아야 함
