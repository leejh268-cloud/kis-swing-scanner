...
목표가는 손익비 2:1(위험의 2배)을 기본으로 하되, 최근 20일 저항선이 그보다 가까우면
저항선에 맞춰 낮춥니다. 여기서 **저항선이 너무 가까워서 손익비 1.3:1도 안 나오는
자리는 신호 자체를 스킵**합니다(`TARGET_MODE = "skip_if_capped"`). 백테스트로 세 가지
방식을 비교한 결과입니다.

| 방식 | 손익비 | 거래 수 | 평균 수익률(전체) |
|---|---|---|---|
| baseline (저항으로 캡만) | 1.04 | 101건 | +0.07% |
| lower_rr (목표를 1.5:1로 낮춤) | 0.94 | 101건 | -0.09% |
| **skip_if_capped (현재 기본값)** | **1.15** | 61건 | **+0.27%** |

바꾸고 싶으면 `scanner/swing_scan.py`의 `TARGET_MODE`를 수정하세요.

## 상대강도(RS) 필터 (신규, 기본 꺼짐)

같은 상승장이어도 지수보다 덜 오르는 종목보다, 지수보다 더 강한 종목(주도주)이
스윙에서 성과가 좋은 경향이 있다는 가설로 추가했지만, **백테스트 결과 오히려
손익비가 악화되어(1.04 → 0.83) 기본적으로 꺼둔 상태입니다.**
과열 필터와 겹치면서 표본이 절반 이하로 줄어든 게 원인으로 보입니다.

다시 켜보고 싶으면 `scanner/rs_filter.py`의 `ENABLED = False`를 `True`로,
`scanner/backtest.py`의 `USE_RS_FILTER`도 `True`로 바꾸고 백테스트로 재검증하세요.

## 시장 레짐 필터 (신규)

코스피/코스닥 지수가 각자의 50일 이동평균선 아래에 있으면("하락장") 해당 시장 종목의
신호는 자동으로 제외됩니다. 아이폰 화면 상단에 코스피/코스닥이 지금 상승장인지
하락장인지 배지로 표시됩니다.

완전히 끄고 싶으면 `scanner/regime_filter.py`의 `HARD_FILTER = True`를 `False`로 바꾸세요
(그러면 신호는 그대로 다 나오고, `market_regime_bullish` 값만 참고용으로 붙습니다).

## 백테스트 (신규)

지금 이 신호 조건이 과거 데이터에서 실제로 승률/손익비가 어땠는지 확인할 수 있습니다.

```bash
python -m scanner.backtest
```

시가총액 상위 150종목(`scanner/backtest.py`의 `SAMPLE_SIZE`) x 약 1.2년치 데이터로
가상매매를 시뮬레이션해서 승률, 평균 수익률, 손익비(Profit Factor) 등을 출력하고
`backtest_trades.csv`로 개별 거래 내역을 저장합니다.

- Profit Factor가 1.0보다 커야 이론적으로 우위가 있는 전략입니다.
- 수수료/세금/슬리피지는 반영되지 않았으니 실제 성과는 이보다 낮게 나올 가능성이 높습니다.
- 결과가 안 좋게 나오면 `scanner/swing_scan.py`의 임계값을 조정한 뒤 다시 돌려서 비교해보세요.

## 스윙 신호 로직 요약

- **추세**: 종가 > 20일선 > 60일선, ADX(14) ≥ 15, 종가가 20일선 대비 +8% 이내 (과열 추격매수 방지)
- **모멘텀**: 최근 3봉 내 MACD 골든크로스, RSI(14) 35~70구간
- **거래량**: 당일 거래량이 최근 20일 평균의 1.3배 이상, OBV가 OBV 20일선 위
- **상대강도**: (기본 꺼짐) 최근 20거래일 수익률이 같은 시장 지수보다 높아야 함
